...
def create_index_html(directory_path, root_path=None):
    ...
    if relative_path != Path():
        current = Path()
        breadcrumb_parts.append(("Compose Bridge Artifacts", "../" * len(relative_path.parts)))
        for part in relative_path.parts:
            current = current / part
            depth = len(relative_path.parts) - len(current.parts)
            breadcrumb_parts.append((part, "../" * depth if depth > 0 else "./"))
    else:
        breadcrumb_parts.append(("Compose Bridge Artifacts", "./"))
...
<head>
    ...
    <title>Phantom Compose Bridge Artifacts - {title_path}</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1><i class="fas fa-cube"></i> Phantom Compose Bridge Artifacts</h1>
