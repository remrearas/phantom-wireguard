#!/usr/bin/env bash
# Bump wireguard-go-bridge version (patch by default).
#
# Updates BOTH version sources atomically in a single commit:
#   - wireguard_go_bridge/__init__.py → __version__
...
#
# Usage:
#   .github/scripts/bump.sh          → 2.1.2 → 2.1.3
#   .github/scripts/bump.sh minor    → 2.1.2 → 2.2.0
#   .github/scripts/bump.sh major    → 2.1.2 → 3.0.0
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
INIT_FILE="${REPO_ROOT}/wireguard_go_bridge/__init__.py"
