...
enum PhantomUIEngine {
    ...
    /// A6: Can this tunnel's toggle be interacted with?
    /// Disabled when another tunnel is active — only one tunnel at a time.
    static func canToggleTunnel(tunnelStatus: TunnelStatus, allStatuses: [TunnelStatus]) -> Bool {
        let anotherActive = allStatuses.contains { $0 != .inactive }
        if tunnelStatus == .inactive && anotherActive { return false }
        return true
    }

    /// A7: Maximum number of tunnel configurations.
    static let maxTunnels = 5

    /// A8: Can a new tunnel be added?
    /// Blocked when tunnel limit is reached or any tunnel is active.
    static func canAddTunnel(tunnelCount: Int, statuses: [TunnelStatus]) -> Bool {
        tunnelCount < maxTunnels && !statuses.contains { $0 != .inactive }
    }

    /// A9: Import submit butonu aktif mi?
    /// Boş text ile import yapılamaz.
    ///
    /// A9: Is the import submit button enabled?
