...
struct InteractionRuleTests {
    ...
    @Test("canToggleTunnel — başka tunnel aktifken inactive toggle disable")
    func canToggleTunnel() {
        // Tek tunnel, inactive → toggle enabled
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .inactive, allStatuses: [.inactive]) == true)
        // Tek tunnel, active → toggle enabled (deactivate için)
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .active, allStatuses: [.active]) == true)
        // İki tunnel: biri active, diğeri inactive → inactive toggle disabled
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .inactive, allStatuses: [.active, .inactive]) == false)
        // İki tunnel: biri activating, diğeri inactive → inactive toggle disabled
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .inactive, allStatuses: [.activating, .inactive]) == false)
        // İki tunnel: biri deactivating, diğeri inactive → inactive toggle disabled (hâlâ geçiş süreci)
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .inactive, allStatuses: [.deactivating, .inactive]) == false)
        // İki tunnel: biri waiting, diğeri inactive → inactive toggle disabled
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .inactive, allStatuses: [.waiting, .inactive]) == false)
        // Aktif tunnel kendi toggle'ını kullanabilir (deactivate)
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .active, allStatuses: [.active, .inactive]) == true)
        // Deactivating tunnel kendi toggle'ını kullanabilir
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .deactivating, allStatuses: [.deactivating, .inactive]) == true)
        // Hepsi inactive → hepsi enabled
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .inactive, allStatuses: [.inactive, .inactive]) == true)
    }

    @Test("canAddTunnel — limit ve status kontrolleri")
    func canAddTunnel() {
        // Boş liste → eklenebilir
        #expect(PhantomUIEngine.canAddTunnel(tunnelCount: 0, statuses: []) == true)
        // 4 tunnel, hepsi inactive → eklenebilir
        #expect(PhantomUIEngine.canAddTunnel(tunnelCount: 4, statuses: [.inactive, .inactive, .inactive, .inactive]) == true)
        // 5 tunnel (limit) → eklenemez
        #expect(PhantomUIEngine.canAddTunnel(tunnelCount: 5, statuses: [.inactive, .inactive, .inactive, .inactive, .inactive]) == false)
        // 3 tunnel ama biri active → eklenemez
        #expect(PhantomUIEngine.canAddTunnel(tunnelCount: 3, statuses: [.inactive, .active, .inactive]) == false)
        // maxTunnels sabiti
...
struct CrossScenarioTests {
    ...
    func scenarioB_tunnelActive() {
        ...
        #expect(PhantomUIEngine.canToggleTunnel(tunnelStatus: .inactive, allStatuses: [status, .inactive]) == false)
