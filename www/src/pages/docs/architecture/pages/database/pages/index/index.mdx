...
Tüm veritabanları SQLite formatındadır ve WAL (Write-Ahead Logging) modunda çalışır. İki kategoriye ayrılır: **kalıcı veriler** (yedekleme kapsamında) ve **çalışma zamanı durumu** (bridge yapıları tarafından yönetilir — ilgili yapının OS ve kernel üzerindeki durumunu yansıtır, taşınabilir veya kullanıcıya özel bilgi içermez).

---

## wallet.db

İstemci havuzu, alt ağ yapılandırması ve DNS ayarlarını içerir.
...
Exit tünel yapılandırmaları ve multihop durumunu içerir.
...
WireGuard arayüz durumunu persist eder. Her arayüz için ayrı bir `device.db` oluşturulur (`/var/lib/phantom/state/db/wireguard/{ifname}/`).
...
Firewall kural grupları, firewall kuralları ve routing kurallarını içerir. Atomik kural uygulaması ve cascade delete destekler.
...
<InlineNotification
    kind="info"
    title="Çalışma zamanı veritabanları"
    subtitle="device.db ve firewall.db daemon başlatıldığında otomatik oluşturulur. Yedekleme kapsamında değildir — bridge'ler tarafından yönetilir."
    hideCloseButton
    lowContrast
/>
