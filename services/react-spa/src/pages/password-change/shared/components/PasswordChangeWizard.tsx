...
import '@pages/totp/shared/components/common.scss';
...
function generatePassword(length = 16): string {
  ...
  for (const byte of array) {
    pw += PASSWORD_CHARS[byte % PASSWORD_CHARS.length];
  }
...
const PasswordChangeWizard: React.FC = () => {
  ...
  const policyChecks: PolicyCheck[] = useMemo(() => [
    { key: 'minLength', label: t.passwordChange.policy.minLength, test: (pw) => pw.length >= 8 },
    { key: 'uppercase', label: t.passwordChange.policy.uppercase, test: (pw) => /[A-Z]/.test(pw) },
    { key: 'lowercase', label: t.passwordChange.policy.lowercase, test: (pw) => /[a-z]/.test(pw) },
    { key: 'digit', label: t.passwordChange.policy.digit, test: (pw) => /\d/.test(pw) },
    { key: 'special', label: t.passwordChange.policy.special, test: (pw) => /[!@#$%^&*()_+\-=\[\]{}|;:'",.<>?/`~\\]/.test(pw) },
  ], [t]);
  ...
  const handleChange = async () => {
    ...
    setLoading(false);
    if (res.ok) {
      // Remove token immediately — backend revoked all sessions
      // React state keeps rendering done screen, but any navigation will hit login
