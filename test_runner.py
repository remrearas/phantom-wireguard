...
Test runner for wstunnel-bridge v2.
...
IMAGE_NAME = "wstunnel-bridge-v2-test:latest"
CONTAINER_PREFIX = "ws-bridge-test"
...
def start_container(client, name: str):
    ...
    container = client.containers.run(
        image=IMAGE_NAME, name=name, detach=True,
        remove=False,
    )
    for _ in range(30):
        container.reload()
...
def write_summary(results_dir: Path, unit_ok: bool, intg_ok: bool,
    ...
    lines = [
        "wstunnel-bridge v2 — Test Results",
        "=" * 50,
...
def main():
    parser = argparse.ArgumentParser(description="wstunnel-bridge v2 test runner")
