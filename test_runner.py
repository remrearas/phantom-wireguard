...
Test runner for wstunnel-bridge v2.1.0.

Three containers:
    ws-bridge-test   — Rust .so + Python code, runs pytest (Docker socket mounted)
    ws-exit-server   — WireGuard server, provides client config
    ws-client        — wstunnel client + WireGuard client, controlled from bridge
...
BRIDGE_IMAGE = "wstunnel-bridge-test:v2.1.0"
EXIT_IMAGE = "wstunnel-bridge-exit:v2.1.0"
CLIENT_IMAGE = "wstunnel-bridge-client:v2.1.0"
NETWORK_NAME = "ws-bridge-test-net"
BRIDGE_NAME = "ws-bridge-test"
EXIT_NAME = "ws-exit-server"
...
def build_image(client, dockerfile: str, tag: str, force: bool = False) -> bool:
...
def start_client(client):
    """Start client container — wstunnel client + WireGuard."""
...
def start_bridge(client, env: dict = None):
    ...
    environment = {
        "WSTUNNEL_BRIDGE_LIB_PATH": "/workspace/libwstunnel_bridge_linux.so",
...
def run_tests(bridge, results_dir: Path, scope: str) -> bool:
    ...
    cmd = (
        ...
        f"-v --tb=short "
        f"--cov=wstunnel_bridge --cov-report=term-missing "
...
def main():
    parser = argparse.ArgumentParser(description="wstunnel-bridge v2.1.0 test runner")
    ...
    if not args.no_build:
        ...
        if not build_image(client, "tests/Dockerfile", BRIDGE_IMAGE, force):
            sys.exit(1)
        if not build_image(client, "tests/exit-server.Dockerfile", EXIT_IMAGE, force):
            sys.exit(1)
        if not build_image(client, "tests/client.Dockerfile", CLIENT_IMAGE, force):
            sys.exit(1)
    ...
    try:
        sync_code(bridge)
        passed = run_tests(bridge, results_dir, scope)
    finally:
        remove_container(client, BRIDGE_NAME)
        remove_container(client, EXIT_NAME)
        remove_container(client, CLIENT_NAME)
    ...
    summary = [
        "wstunnel-bridge v2.1.0 — Test Results",
        "=" * 50,
