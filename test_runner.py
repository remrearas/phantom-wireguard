...
Test runner for firewall-bridge v2.1.0.

Three containers:
    bridge-test  — Rust .so + Python code, runs pytest (Docker socket mounted)
...
BRIDGE_IMAGE = "firewall-bridge-test:v2.1.0"
EXIT_IMAGE = "firewall-bridge-exit:v2.1.0"
CLIENT_IMAGE = "firewall-bridge-client:v2.1.0"
NETWORK_NAME = "fw-bridge-test-net"
BRIDGE_NAME = "fw-bridge-test"
EXIT_NAME = "fw-exit-server"
CLIENT_NAME = "fw-client"
...
def start_bridge(client, env: dict = None):
    ...
    environment = {
        "FIREWALL_BRIDGE_LIB_PATH": "/workspace/libfirewall_bridge_linux.so",
...
def run_tests(bridge, results_dir: Path) -> bool:
    ...
    cmd = (
        ...
        "-v -s --tb=short "
        "--log-cli-level=INFO "
        "--cov=firewall_bridge --cov-report=term-missing "
...
def main():
    ...
    summary = [
        "firewall-bridge v2.1.0 — Test Results",
        "=" * 50,
